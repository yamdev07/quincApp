...
return new class extends Migration
{
    ...
    /**
     * Safely populate tenant_id for sales table
     */
    private function populateTenantId(): void
    {
        // Check if we have sales without tenant_id
        $nullTenantCount = DB::table('sales')->whereNull('tenant_id')->count();
        ...
        if (Schema::hasColumn('users', 'tenant_id')) {
            try {
                ...
                $remainingCount = DB::table('sales')->whereNull('tenant_id')->count();
                if ($remainingCount === 0) {
                    return;
                }
            } catch (\Exception $e) {
                ...
                if (app()->environment('testing')) {
                    \Log::warning('Could not update tenant_id from users for sales: ' . $e->getMessage());
                }
            }
        }
        ...
        if ($firstTenant) {
            ...
            if (app()->environment('testing')) {
                ...
                DB::table('sales')->whereNull('tenant_id')->update(['tenant_id' => $tenantId]);
            }
        }
    }
